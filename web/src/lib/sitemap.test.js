...
const xml = sitemapXml({ matches, leagues }, ORIGIN);
const urls = [...xml.matchAll(/<url><loc>([^<]*)<\/loc>(?:<lastmod>([^<]*)<\/lastmod>)?<\/url>/g)]
  .map(([, loc, lastmod]) => ({ loc, lastmod }));
const PAGES = STATIC_PATHS.length + leagues.length + matches.length;
...
test('league pages at their address with their lastmod', () => {
  assert.deepEqual(urls.slice(STATIC_PATHS.length, STATIC_PATHS.length + leagues.length), [
...
test('match pages at their canonical address with the API lastmod', () => {
  assert.deepEqual(urls.slice(STATIC_PATHS.length + leagues.length), [
...
test('text is escaped', () => {
  const odd = sitemapXml({ matches: [{ fixture_id: 1, slug: 'a&b<c', lastmod: 'x' }], leagues: [] }, ORIGIN);
