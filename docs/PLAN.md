...
| OPEN-6 (recal pooling E1–E3) | Open, with **three converging lines of evidence** | E0 separates from E1–E3 on three unrelated measurements, none designed to test pooling: (a) draw deficit **+0.01 pts in E0** vs +0.96 to +1.24 in E1–E3 (P0-2, re-measured); (b) share of the market's edge the base head captures, **0.89 in E0** vs 0.51–0.64 (`BASELINE.md` §2); (c) the season-boundary shrink **harms E0 1X2** while helping E1–E3 O/U (`BASELINE.md` §5). Still decide by `y ~ logit(p) * division` in P3, but the prior is now firmly "E0 separate, E1–E3 possibly poolable". **Separately**, the pre-close/closing axis is real but small (−0.00246 pooled) and **absent in E1** — splitting on it halves the sample per Platt fit and needs its own justification, not inheritance |
...
| **NEW-1** EC in the joint fit? | **RESOLVED — include** | P1 H5 gate: pooled **−0.00290** [−0.00427, −0.00173] on goal deviance. Effect concentrates exactly where the mechanism predicts — **E3 −0.0112**, and **−0.0607 for clubs promoted out of EC last season**. Excluded from every served market and every scoring set. Note the gate *reversed* between the two P1 runs: at the over-regularised α the buggy 1-SE rule picked, EC appeared to hurt. `BASELINE.md` §3 |
| **NEW-4** 2015-16 season was missing | **RESOLVED 2026-07-28** | `data/play_history/201516/` had been a byte-identical copy of `201415/`. Owner supplied the genuine files; store rebuilt (2015-08-07 → 2016-05-17, zero duplicate fixtures, `validate()` clean, Leicester tops E0 as it should). P1 fully re-issued on all thirteen seasons: **half-life 500 → 400**, every other decision held. Two findings turned out to be artifacts of the defect — the H4 interaction and the decay optimum's COVID sensitivity — both of which had CIs excluding zero. Guards now in `build.validate()`. **Outstanding: the P0 dispersion results were also measured on the corrupted corpus and have not been re-measured.** `BASELINE.md` §4.2 |
