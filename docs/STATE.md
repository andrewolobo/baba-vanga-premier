...
file is stale — fix this file. Updated **2026-09-07**.
...
| cycle | sync → calendar → serve → tips → results → grade; exit **0** clean / **2** look / **1** failed. `results` (BBC full-time scores, `BVP_BBC_RESULTS=1`) settles tips before football-data's file exists; `grade` reconciles once it does. `results` also runs alone every two hours (`bvp-results.timer`, 2026-08-21) | `services/run_cycle.py` |
| API | reads, plus the three account writes (B25, `/auth/google`, `/auth/logout`, `/me/phone` — `users`/`user_sessions` only, pinned by test); `/tips`, `/tips/results`, `/tips/record` (no P&L on the wire; headline pools every rule version since 2026-08-21, `by_rule` splits it; results carry the scoreline the grader settled from — migration 006) | `api/main.py` |
| site | one page: calls, last results, record; each call opens a drawer showing the next-likeliest markets behind it (B22, display only; the ranked-results reading was removed 2026-08-20); the settled list has its own league filter and last-12 ⇄ show-all toggle, and each settled card can show the score it was graded from plus the claimed probability — behind a "Scores & claims" toggle, off by default; the rule version and per-version table are owner-only via `/?owner=1` (`$lib/owner.js`, 2026-08-20); installable PWA — manifest linked, service worker precaches the shell and keeps `/tips` + `/fixtures` readable offline (2026-09-04) | `web/` |
...
| **ops** | **backup timer enabled and restore-drilled 2026-09-04** (`pg_dump` daily 06:30 UTC, local only); no off-VM copy; no alerting; HTTP only | set `BVP_BACKUP_CONTAINER`; dead-man's-switch ping from `run_cycle.sh` (owner supplies URL); TLS when a domain exists |
| **B25** Google sign-in + one-time phone | **built 2026-09-07, not deployed** — `AUTH_PLAN.md`: `002_users.sql` on the dev store, `api/auth.py`, five account routes (the API's only writes, pinned), `tests/test_auth.py` 28, **702 pass**; GIS button, header state and the phone gate in `+layout.svelte`, **33 web tests**, build clean, 22-check click-through; nginx limits, unit stub, `backup.sh`, lock pins, docs all in the tree; the real button renders locally with the owner's client id; nothing gated, the site is unchanged while the client id is empty | **owner runs `AUTH_PLAN.md` §9.1 on the VM** (commit first; limits file before the re-render; id on the unit copy; `deploy.sh`; real sign-in over HTTPS) |
...
- Gate ledger: `trials.count_configurations(conn)` — **112 / 69 / 202** at last
  ...
  is behind.
- Tests: `pytest -q` — **702 pass** at last run (2026-09-07, B25 phase A,
  Python 3.11, on the owner's Postgres), plus **33** in `cd web && npm test`; the prose has been stale
  before.
