...
| **Postgres move** (for sign-in) | **Phases A and B done 2026-09-04** — tree speaks Postgres; copier built and tested; local store copied into `bvp` on the owner's instance and **verified identical, API JSON byte-identical** against the SQLite commit. **`bvp` is now this machine's ledger authority**; `db/premier.db` frozen; **656 pass, no skips**. **Production snapshot rehearsed too: verify identical, JSON byte-identical.** Nothing on the VM yet | **Phase C cutover, between matchdays** — checklist in `POSTGRES_PLAN.md` §4 |
