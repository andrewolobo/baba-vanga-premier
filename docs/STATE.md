...
| item | state | next |
| --- | --- | --- |
| **SEO** (`SEO_PLAN.md`) | reviewed 2026-09-16; site added to Search Console by the owner; **1.1 robots.txt + 1.2 real 404s committed (`7b1e6b9`)**, owner deploying (not confirmed here); **1.3 `X-Robots-Tag` on `/api/` + API docs closed committed (`5706458`)**; **1.4 www → apex template built, uncommitted** | deploy 1.3; 1.4 on the VM: render template, expand the cert to www **with the nginx reload hook** (`DEPLOY.md` §5.3 — no renewal ever reloaded nginx before); then 1.5–1.8; pages lack nosniff/referrer headers (`SEO_PLAN.md` F8, pre-existing, not SEO); decisions D1–D11 open (D1 decides Phase 2) |
