...
| **Postgres move** (for sign-in) | **Phase A complete 2026-09-04** — the whole tree speaks Postgres (psycopg 3, `001_baseline.sql`, clone-per-test harness, `pg_dump` backup, units carry the URL); **645 pass, 2 skipped** on the owner's instance. Nothing on the VM yet; `db/premier.db` still the ledger authority | Phase B: the copier (`--survey/--copy/--verify`), rehearsed on the local store then on a production snapshot with the endpoint JSON diff — `POSTGRES_PLAN.md` §4 |
